...
namespace AutoExile.Modes.BossEncounters
{
    ...
    public class FearEncounter : IBossEncounter
    {
        ...
        public bool SuppressCombatPositioning => _phase == FearPhase.WaitForVulnerable || _phase == FearPhase.WaitingForLoot;
        ...
        public BossEncounterResult Tick(BotContext ctx)
        {
            ...
            if (_bossEntity != null && _bossEntity.IsAlive) _bossWasAlive = true;
            ...
            if (_bossWasAlive && _phase != FearPhase.WaitingForLoot && (_bossEntity == null || !_bossEntity.IsAlive))
            {
                // Cache death position for loot navigation
                if (!_bossDeathPos.HasValue && _bossEntity != null) _bossDeathPos = _bossEntity.GridPosNum;
                else if (!_bossDeathPos.HasValue) _bossDeathPos = new Vector2(206, 306); // fallback to spawn pos
            ...
            switch (_phase)
            {
                case FearPhase.Approaching: return TickApproaching(ctx, gc, playerGrid);
                case FearPhase.WaitForVulnerable: return TickWaitForVulnerable(ctx, gc, playerGrid);
                case FearPhase.Fighting: return TickFighting(ctx, gc, playerGrid);
                case FearPhase.WaitingForLoot: return TickWaitingForLoot(ctx, gc, playerGrid);
        ...
        private BossEncounterResult TickApproaching(BotContext ctx, GameController gc, Vector2 playerGrid)
        {
            ...
            if (_isReentry)
            {
                ...
                if (_bossWasAlive && (_bossEntity == null || !_bossEntity.IsAlive)) return BossEncounterResult.InProgress; // kill detection above handles transition
            ...
            if (distToDps > 10)
            {
                if (!ctx.Navigation.IsNavigating) ctx.Navigation.NavigateTo(gc, dpsPos);
        ...
        private BossEncounterResult TickReentryApproach(BotContext ctx, GameController gc, Vector2 playerGrid)
        {
            ...
            if (!transition.IsTargetable)
            {
                ...
                if (playerGrid.Y > 150)
                {
                    if (!ctx.Navigation.IsNavigating) ctx.Navigation.NavigateTo(gc, TransitionApproachPos);
            ...
            // After successful transition, bot will be in boss room — detect via position jump or boss becoming close
        ...
        private BossEncounterResult TickWaitForVulnerable(BotContext ctx, GameController gc, Vector2 playerGrid)
        {
            ...
            if (distToDps > 15 && !ctx.Navigation.IsNavigating) ctx.Navigation.NavigateTo(gc, dpsPos2);

            var elapsed = (DateTime.Now - _phaseStartTime).TotalSeconds;
            Status = _bossEmerged ? $"Emerged — flasks active! ({elapsed:F1}s)" : $"Pre-laying traps ({elapsed:F1}s)";
        ...
        private BossEncounterResult TickFighting(BotContext ctx, GameController gc, Vector2 playerGrid)
        {
            ...
            if ((DateTime.Now - _lastOrbitBlink).TotalMilliseconds >= OrbitBlinkIntervalMs)
            {
                ...
                if (blinkSkill != null)
                {
                    ...
                    if (screenPos.X > 0 && screenPos.X < windowRect.Width && screenPos.Y > 0 && screenPos.Y < windowRect.Height)
                    {
        ...
        private BossEncounterResult TickWaitingForLoot(BotContext ctx, GameController gc, Vector2 playerGrid)
        {
            ...
            if (distToLoot > 15 && !ctx.Navigation.IsNavigating) ctx.Navigation.NavigateTo(gc, lootPos);
        ...
        private MovementSkillInfo? FindReadyMovementSkill(BotContext ctx)
        {
            foreach (var ms in ctx.Navigation.MovementSkills)
            {
                ...
                if (ms.MinCastIntervalMs > 0 && (DateTime.Now - ms.LastUsedAt).TotalMilliseconds < ms.MinCastIntervalMs) continue;
                return ms;
            }
            return null;
        }
        ...
        public void Render(BotContext ctx)
        {
            ...
            if (_bossEntity != null)
            {
                ...
                {
                    var color = _bossEntity.IsAlive ? (_bossVulnerable ? SharpDX.Color.Red : SharpDX.Color.Yellow) : SharpDX.Color.LimeGreen;
                    var label = _bossVulnerable ? "FEAR BOSS (DPS!)" : _bossEmerged ? "FEAR BOSS (flasks!)" : _bossEntity.IsAlive ? "FEAR BOSS (invuln)" : "FEAR BOSS (dead)";
            ...
            if (_phase == FearPhase.WaitingForLoot && _bossDeathPos.HasValue)
            {
                ...
                if (screen.X > 0 && screen.X < 2400) g.DrawText("LOOT HERE", screen + new Vector2(-30, -20), SharpDX.Color.Gold);
            }
